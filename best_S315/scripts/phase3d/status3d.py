...
# -*- coding: utf-8 -*-
"""
status3d.py —— Phase 3d 每 10 分鐘覆寫 status.md (手機可讀)
讀: ~/hadwiger/phase3d/{START.txt, STEP.txt}, runs/*/status.json (probe3d), 當前 campaign 嘅 cnc status.json, keep 目錄, 磁碟, runs/*/finalize_status.json
寫: ~/hadwiger/phase3d/status.md + Windows Desktop\\spindle\\phase3d\\status.md
用法: python3 status3d.py [--interval 600] [--once]
"""
import os, sys, json, time, argparse, glob

PH = "/home/user/hadwiger/phase3d"
WIN = ["/mnt/c/Users/user/Desktop/spindle/phase3d/status.md"]
SHM = "/dev/shm"
RECORD_G3P = 1441
...
def render():
    ...
    for _, st, _ in sts:
        if st and st.get("best"):
            best = st["best"]
    title = "# Phase 3d status (最後一刀: 一刀剪 80 粒, 目標 |G₃′| < %d)" % RECORD_G3P
    if rec and confirmed:
        ...
        title = "# **RECORD_CANDIDATE (候選, finalize 獨立重驗中)** —— probe3d 到咗 |G₃′| = %s < %d; 等全部 leaf 證明重驗 + cake_lpr + 封存" % ((best or {}).get("G3p_n"), RECORD_G3P)
    elif vfail:
        title = "# **!! 獨立重驗唔過** —— finalize3d verify all_ok=False (VERIFY_FAILED.md), 要人手睇"
    elif best:
        title = "# Phase 3d: 有進步 (|G₃′| = %s, 未到 %d)" % (best.get("G3p_n"), RECORD_G3P)
    elif fin and fin[-1].get("done"):
        title = "# Phase 3d status —— 完成 (%s); 報告 phase3d/phase3d_results.md; 冇背景任務" % fin[-1].get("finished")
    L = [title, "", "更新: %s (每 10 分鐘覆寫)" % time.strftime("%Y-%m-%d %H:%M:%S"),
         "Phase 3d 用時 (由 launch 起): %s" % ("%.1f h" % ((now - start) / 3600) if start else "?"), "", "## 當前步驟", step, ""]
    ...
    L += ["## 磁碟", "- D: 剩 %.0f GB (keep + 封存所在, 要 ≥ 300) | C: 剩 %.0f GB (要 ≥ 25) | WSL ext4 剩 %.0f GB | /dev/shm (證明暫存) 剩 %.1f GB, 暫存中 %.2f GB" % (
        dD or -1, dC or -1, dR or -1, dS or -1, du(os.path.join(SHM, "p3d1_final"))), ""]
    warn = []
    if dD is not None and dD < 300:
        warn.append("!! D: 剩餘 < 300 GB → probe3d 唔會開新證書")
    ...
    L.append("## 最後一刀 (run ID 獨一)")
    found = False
    for rid, st, d in sts:
        if not st:
            L.append("- %s: (冇 status.json) log 尾: %s" % (rid, " | ".join(tail(os.path.join(d, "probe3d.log"), 2))[:300])); found = True; continue
        found = True
        ...
        seed = st.get("seed") or {}
        L.append("- **%s** (接力 %s %s: S_acc %s 粒 → |G₃′| %s): 證書 %s/%s | 目標 |S| %s ⇒ |G₂′| %s, |G₃′| %s | 用時 %.2f h / %.0f h | **CPU %.1f / %.0f CPU-h** (剩 %.1f) | 排除 %s 粒%s%s%s" % (
            rid, seed.get("run_id"), seed.get("last_certified_tag"), 270, seed.get("G3p_n"), st.get("attempts_done"), st.get("attempts_max"),
            st.get("target_removed"), st.get("target_G2p"), st.get("target_G3p"), (st.get("elapsed_s") or 0) / 3600, (st.get("total_cap_s") or 0) / 3600,
            st.get("cpu_h_used") or 0, st.get("cpu_cap_h") or 0, st.get("cpu_h_remaining") or 0, st.get("excluded"),
        ...
        if st.get("best"):
            b = st["best"]
            L.append("  - **最佳認證圖: %s — |G₂′| %s 點 %s 邊, |G₃′| %s 點 %s 邊 (%s)**" % (
                b.get("tag"), b.get("G2p_n"), b.get("G2p_m"), b.get("G3p_n"), b.get("G3p_m"),
                ("**< %d**" % RECORD_G3P) if (b.get("G3p_n") or 10 ** 9) < RECORD_G3P else "未到 %d" % RECORD_G3P))
        ...
        if cur and not st.get("stop_reason"):
            L.append("  - 當前嘗試 %s: 階段 %s, 剪 %s 粒, 開始 %s, 輪上限 %.2f h, keep-dir %s (暫存 %.2f GB, 已搬 %.1f GB)" % (
                ...
                du(os.path.join(SHM, "p3d1_final"), "cnc_%s_" % cur.get("tag")), du(cur.get("keep_dir") or "/nonexistent")))
        ...
        for x in st.get("attempts_list", []):
            L.append("  - %s: %s | 剪 %s 粒 → |G₂′| %s |G₃′| %s | %s cube, mean s+v %s s, 最長 %s s | %.2f h%s%s%s" % (
                x.get("tag"), x.get("status"), x.get("removed"), x.get("G2p_n"), x.get("G3p_n"), x.get("n_cubes"), x.get("mean_sv_s"), x.get("max_solve_s"),
                (x.get("wall_s") or 0) / 3600, (" (+延長 %.2f h)" % ((x.get("extended_s") or 0) / 3600)) if x.get("extended_s") else "",
                (" | |B| = %s" % x.get("B_size")) if x.get("B_size") else "", (" | 認證 l3 %s / 5 色 %s" % (x.get("l3_ok"), x.get("col5_ok"))) if x.get("status") == "certified" else ""))
        for b in st.get("B_sets", []):
            L.append("  - 擋路集 %s: |B| = %s, B = %s" % (b.get("tag"), b.get("size"), b.get("B")))
    if not found:
        L.append("- 無")
    L.append("")
    for f in fin:
        L += ["## 收爐 (finalize3d)", "- %s" % f.get("stage"), "- 開始 %s%s" % (f.get("started"), (", 完 %s" % f["finished"]) if f.get("finished") else "")]
