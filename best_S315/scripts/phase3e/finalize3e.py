...
finalize3e.py —— Phase 3e 收爐 / RECORD 程序 (probe3e 停機之後自動跑; 幂等, 可重跑)
  **機械生成**: 由 phase3d/finalize3d.py 經 phase3e/make_finalize3e.py 改名 + 加「輪」帳而成; diff 喺 phase3e/finalize3e.diff。唔好手改呢個檔, 改 make_finalize3e.py 再生成。
  ...
  5. 每輪細檔 → ~/hadwiger/release_v1_1_staging/reduction/phase3e_G2_final/ + 報告 phase3e_results.md + facts.json (g2shrink3e.*) + DECISION.md 路線 A + MEMORY
  唔准公開: 冇 git push, 冇 Zenodo, 冇 email.
用法: python3 finalize3e.py --run ~/hadwiger/phase3e/runs/p3e1_step [--workers 14] [--dry-run] [--force-verify] [--frac 0.05]
"""
...
class Fin:
    def __init__(self, a):
        ...
        if not self.st.get("finished"):
            tail = ""
            try:
                tail = " | ".join([l.strip() for l in open(os.path.join(self.rdir, "probe3e.log"), errors="ignore").read().splitlines() if l.strip()][-3:])[:600]
            except Exception:
                pass
            self.st["stop_reason"] = "!! probe3e 未正常結束 (state.finished 冇): %s || probe3e.log 尾: %s" % (self.st.get("stop_reason"), tail)
    ...
    def step(self, msg):
        ...
        try:
            open(os.path.join(PH3E, "STEP.txt"), "w").write("Phase 3e 收爐 (finalize3e): " + msg)
        except Exception:
            pass
    ...
    def final_attempt(self):
        ...
        assert best, "!! state.best 冇 —— probe3e 冇拎到新證書 (stop: %s); 冇嘢好封存" % self.st.get("stop_reason")
    ...
    def archive(self, fa, vres, xres, c5):
        ...
        for f in ("state.json", "attempts.json", "rounds.json", "ledger_g2.md", "status.json", "probe3e.log", "finalize_status.json"):
        ...
        for sub, src in (("phase3e", PH3E), ("phase3d", PH3D), ("phase3c", PH3C), ("phase3b", PH3B)):
        ...
        for stale in ("RECORD_CANDIDATE.md", "RECORD_CANDIDATE_BLOCKED.md", "VERIFY_FAILED.md"):    # 審查: 封存目錄以外嘅副本都要清, 免得舊判斷留低誤導
            for base_ in (F, PH3E, os.path.join(WIN, "phase3e")):
        ...
        if not self.vok:
            ...
            for p_ in (os.path.join(F, "VERIFY_FAILED.md"), os.path.join(PH3E, "VERIFY_FAILED.md"), os.path.join(WIN, "phase3e", "VERIFY_FAILED.md")):
                write(p_, txt)
        if self.rec_ok:
            rec_md = self.write_record_candidate(F, fa, vres, xres, c5)
        elif fa["G3p_n"] < RECORD_G3P:
            txt = ("# RECORD_CANDIDATE_BLOCKED.md (%s)\n\nprobe3e reached |G3'| = %d < %d, but the independent re-verification did NOT pass "
            ...
            for p_ in (os.path.join(F, "RECORD_CANDIDATE_BLOCKED.md"), os.path.join(PH3E, "RECORD_CANDIDATE_BLOCKED.md"), os.path.join(WIN, "phase3e", "RECORD_CANDIDATE_BLOCKED.md")):
                write(p_, txt)
...
exit $RC
""" % (fa["leaves"], S, fa["leaves"], tag, tag, S)
    ...
    def write_readme(self, F, fa, vres, xres, c5, n_leaf, n_aud):
        ...
        L = [title, "",
             ("Archived %s by `scripts/phase3e/finalize3e.py` (run `%s`, certificate `%s`, seeded from Phase 3c `%s` %s). Author of the project: King Tat Wong (Amber). "
    ...
    def write_record_candidate(self, F, fa, vres, xres, c5):
        ...
        L = ["# RECORD_CANDIDATE.md —— |G₃′| = %d < %d (Phase 3e, %s)" % (G3n, RECORD_G3P, self.now), "",
             "## 廣東話摘要 (俾 Amber)", "",
        ...
        cp(p, PH3E); cp(p, os.path.join(WIN, "phase3e"))
        return p

    # ---------------- 4. reduction/phase3e_G2_final ----------------
    def stage_small(self):
        self.step("封存細檔 → %s/phase3e_G2_final (+ 重算 reduction/SHA256SUMS)" % STAGE)
        if self.a.dry_run:
            return {"dry_run": True}
        D = os.path.join(STAGE, "phase3e_G2_final")
        for f in ("state.json", "attempts.json", "rounds.json", "ledger_g2.md", "status.json", "probe3e.log", "finalize_status.json"):
        ...
        for f in glob.glob(os.path.join(PH3E, "*.py")) + glob.glob(os.path.join(PH3E, "*.sh")):
            cp(f, os.path.join(STAGE, "scripts", "phase3e"))
        best = self.st.get("best") or {}
        L = ["# PHASE3E_README.md — Phase 3e: the last cut (run %s; staged %s; NOT released)" % (self.rid, self.now), "",
        ...
        write(os.path.join(D, "PHASE3E_README.md"), "\n".join(L) + "\n")
        ...
        for f in glob.glob(os.path.join(STAGE, "scripts", "phase3e", "*")):
            cp(f, os.path.join(WIN_STAGE, "scripts", "phase3e")); n_m += 1
    ...
    def report(self, fa, vres, xres, c5, arch, stg):
        self.step("寫 phase3e_results.md")
        ...
        L = ["# phase3e_results.md —— Phase 3e: 小步慢行 (每輪剪 %s 粒; %d 輪, %d 輪 certified, 淨剪 %s 粒; run %s, 接力 Phase 3c %s)" % (
             ...
             "定稿 %s (自動收爐 finalize3e.py). 開跑 %s; probe3e 停 %s; Phase 3e wall %.2f h / %.0f h; **CPU-h (Σ 證書 wall × %d) %.1f / %.0f**." % (
        ...
        if fa:
            L += ["## 最終認證圖", "",
                  ...
                  "- 細檔 `%s/phase3e_G2_final/`(PHASE3E_README.md;reduction/ 共 %s 檔,`sha256sum -c` rc=%s),鏡像 Windows。" % (STAGE, stg.get("reduction_files"), stg.get("check_rc")), ""]
        L += ["## 老實講明", "",
        ...
        txt = "\n".join(L)
        if self.a.dry_run:
            write(os.path.join(PH3E, "selftest", "phase3e_results_dryrun.md"), txt); self.save(report="dry-run"); return txt
        write(os.path.join(PH3E, "phase3e_results.md"), txt); write(os.path.join(WIN, "phase3e", "phase3e_results.md"), txt)
        self.save(report=os.path.join(WIN, "phase3e", "phase3e_results.md"))
        return txt

    def facts(self, fa, vres, xres, c5, arch, stg):
        self.step("facts.json (g2shrink3e.*)")
        fp = os.path.join(WIN, "paper", "facts.json")
        fp_out = os.path.join(PH3E, "facts_dryrun.json") if self.a.dry_run else fp
        try:
            ...
            add("g2shrink3e.run_id", self.rid, None, "phase3e/runs/%s/state.json" % self.rid)
            add("g2shrink3e.seed", st.get("seed"), None, "phase3e/runs/%s/state.json (Phase 3c r09a provenance: bundle/l3 sha)" % self.rid)
            add("g2shrink3e.params", {"step": (st.get("config") or {}).get("step"), "min_step": (st.get("config") or {}).get("min_step"),
                ...
                None, "Amber's Phase 3e mega-prompt 2026-09-08; probe3e.py argv")
            add("g2shrink3e.attempts", [{k: x.get(k) for k in ("tag", "removed", "status", "n_cubes", "leaves", "mean_solve_s", "median_solve_s", "mean_sv_s", "max_solve_s", "wall_s",
                ...
                None, "phase3e/runs/%s/attempts.json" % self.rid)
            add("g2shrink3e.B_sets", st.get("B_sets"), None, "phase3e/runs/%s/state.json (each B has an edge-checked 4-colouring of G2 − B with col(u) != col(v))" % self.rid)
            ...
            add("g2shrink3e.cpu_h_used", st.get("cpu_h_used"), "CPU-h (wall×%d)" % self.W, "phase3e/runs/%s/state.json" % self.rid)
            add("g2shrink3e.wall_h", round((st.get("elapsed_s") or 0) / 3600, 2), "h", "phase3e/runs/%s/state.json" % self.rid)
            if fa:
                add("g2shrink3e.S_final", fa["S"], None, "phase3e/runs/%s/state.json (S_acc)" % self.rid, "certified removable set of G2 vertices")
                add("g2shrink3e.G2p_final", {"n": fa["G2p_n"], "m": fa["G2p_m"]}, None, "%s/L2L3/l3_final/summary.json (exactfield --complete)" % arch.get("dir"))
                add("g2shrink3e.G3p_final", {"n": fa["G3p_n"], "m": fa["G3p_m"], "below_%d" % RECORD_G3P: fa["G3p_n"] < RECORD_G3P, "previous_ours": PREV_G3P},
                ...
                add("g2shrink3e.final_certificate", {"tag": fa["tag"], "base_sha256": fa["rec"]["cnf"]["cnf_sha256"], "leaves": fa["leaves"],
                    ...
                    None, "phase3e/runs/%s/attempts.json" % self.rid)
                add("g2shrink3e.verify_final", {k: v for k, v in vres.items() if k in ("all_ok", "base_sha_consistent", "leaf_count_consistent", "leaf_ids_are_all_root_cubes", "cover_reverify", "audit_reverify", "exactfield_complete_G3p_again", "t_s")}
                ...
                add("g2shrink3e.rebuild_base_from_S", self.rb, None, "finalize3e.py: buildg2.py --remove <S> 重跑, sha 對封存嘅 base.cnf", "closes the loop 'the CNF really is G2 minus S'", verified=bool(self.rb.get("ok")))
                add("g2shrink3e.crosscheck_cake_lpr", {k: v for k, v in (xres or {}).items() if k not in ("records", "sampled_ids", "bad")}, None, "%s/L1pp/crosscheck.json" % arch.get("dir"))
                add("g2shrink3e.five_colouring", c5, None, "%s/L1pp/col5_check.json + COL5/" % arch.get("dir"))
                add("g2shrink3e.record_candidate", self.rec_ok, None, "phase3e/runs/%s/state.json (probe3e record_candidate=%s) AND finalize independent re-verification (verify_final %s, cake_lpr %s, 5-col %s)" % (
                ...
                add("g2shrink3e.archive", arch, None, "finalize3e.py (SHA256SUMS)")
                add("g2shrink3e.staging", stg, None, "finalize3e.py (reduction/SHA256SUMS)")
            if fa:
                add("g2shrink3e.vs_heule_1441", {"ours_G3p_n": fa["G3p_n"], "heule_1441": RECORD_G3P, "below": fa["G3p_n"] < RECORD_G3P, "our_previous": PREV_G3P,
                    ...
                    None, "derived from g2shrink3e.G3p_final and record.heule_1441", verified=bool(self.vok))
            F["generated"] = time.strftime("%Y-%m-%d %H:%M:%S")
            if "Phase 3e additions (g2shrink3e.*)" not in (F.get("note") or ""):
                F["note"] = (F.get("note") or "") + " | Phase 3e additions (g2shrink3e.*) by phase3e/finalize3e.py."
        ...
        try:
            ...
            para = ("%s(%s;run %s,接力 Phase 3c r09a;Phase 3d 一刀剪 80 三次全 SAT ⇒ 退返細步幅:每輪剪 %s 粒、每輪最多 2 張證書、兩張都 SAT 就步幅減半(20→10→5)、排除集逐輪重置;"
                    ...
                    "每輪:%s;嘗試 %s;CPU-h %.1f / %.0f,wall %.2f h;停機原因:%s。%s報告 `phase3e/phase3e_results.md`。**未公開、未寄信、未 push、未上 Zenodo。**" % (
                        ...
                        ("全套證書(含全部 leaf proofs)封存 `%s`(D:),細檔 `release_v1_1_staging/reduction/phase3e_G2_final/`;" % arch.get("dir")) if fa else ""))
            if marker in s:
                s = re.sub(r"\*\*Phase 3e 小步慢行.*?(?=\n\n)", lambda m: para, s, count=1, flags=re.S)
            else:
                anchor = "\n\n---\n\n## 路線 B"
            ...
            if not self.a.dry_run:
                ...
                write(os.path.join(PH3E, "DECISION_dryrun.md"), s)
    ...
    def memory(self, fa, vres, xres, arch):
        ...
        try:
            ...
            if not self.a.dry_run:
                ...
                write(os.path.join(PH3E, "mem_dryrun_no-spindle-hardness.md"), s2)
            ...
            if not self.a.dry_run:
                ...
                write(os.path.join(PH3E, "mem_dryrun_MEMORY.md"), "\n".join(lines))
    ...
    def run(self):
        if not self.st.get("best"):
            ...
            self.step("收爐完 (冇新證書, %s)" % self.fs["finished"]); log("FINALIZE3E DONE: 冇新證書"); return
        ...
        log("FINALIZE3E DONE: vok=%s, record_candidate=%s, archive %s" % (self.vok, self.rec_ok, arch.get("dir")))
        if not self.vok:
            sys.exit(4)
